...
public class TsurugiBatchInsertPutBatch extends TsurugiKvsBatchInsert {

    public TsurugiBatchInsertPutBatch(PluginTask task, TsurugiOutputConnector connector, Optional<MergeConfig> mergeConfig) {
        super(task, connector, mergeConfig);
    }

    @Override
    protected int[] executeBatch() throws IOException, ServerException {
        return executor.executeBatch(tableName, recordList);
    }
}
